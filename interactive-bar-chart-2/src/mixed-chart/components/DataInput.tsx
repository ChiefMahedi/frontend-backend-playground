    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (isValid) {
          ...
          toast.error("Invalid CSV format. Ensure headers match: Date, Product, MonthSales.", {
            position: "top-right"
          });
        }
      },
  ...
  return (
    <>
      <div className='flex flex-col gap-3  justify-center  items-center shadow-xl	 w-64 h-80 p-3 border border-gray-300 rounded'>
        <FontAwesomeIcon size="2x" icon={faChartColumn} className='bg-blue-800 text-white p-1  rounded ' />
        <h1 className='font-bold text-lg	'>Interactive Bar Chart</h1>
        <p className='text-xs border  rounded  border-blue-300 p-3'><span className='font-bold'>Note:</span> The file must be a .csv file and must contain only these three Columns: Date, Product, and MonthSales of type string, string, and number respectively</p>
